...
class blog {
    ...
    //Read one category
	public function read_recent_blog(){
		$sql = "SELECT * FROM $this->table 
				WHERE f_post_status = 1 ORDER BY n_blog_post_id DESC LIMIT 4";
    ...
    //Read one category
	public function read_blog_by_search($key){
		$sql = "SELECT * FROM $this->table JOIN blog_tags ON blog_tags.n_blog_post_id = $this->table.n_blog_post_id
				WHERE f_post_status = 1 AND (v_post_title LIKE '%$key%' OR v_tag LIKE '%$key%')";
	...
	public function read_blog_by_category(){
		$sql = "SELECT * FROM $this->table
				WHERE f_post_status = 1 AND n_category_id = :category_id";
